...
If your project requires vendoring the source code for the tools, then replace the shellouts with pure Go API calls. A Mage file can issue these API calls in a way that naturally more naturally with `go mod`. At which point you can remove that command line tool from your accio.yaml. Finally, use `go mod vendor`.
