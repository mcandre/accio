...
If your project requires vendoring the source code for the tools, then replace the shellouts with invoke pure Go API calls, such as from a Mage file. By modeling the dependency in terms of a build constraint this way, naturally integrates with `go mod`. At which point you can remove that tool from your accio.yaml. Finally, use `go mod vendor`. For some Go tools it takes a lot of effort to lib-ize a command line tool. In that case, establish an independent fork of the tool's primary repository, and override the package's URL field in your accio configuration to point to your fork.
